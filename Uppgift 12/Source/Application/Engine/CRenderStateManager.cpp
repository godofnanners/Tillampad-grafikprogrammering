...
CRenderStateManager::CRenderStateManager()
{
	...
	myRasterizerStates = { nullptr };
	myBlendStates = { nullptr };
	myDepthStencilStates = { nullptr };
	mySamplerStates = { nullptr };
}
...
bool CRenderStateManager::Init(CDirectX11Framework* aFramework)
{
	...
	if (!CreateBlendStates(aFramework->GetDevice()))
		return false;
	if (!CreateDepthStencilState(aFramework->GetDevice()))
		return false;
	if (!CreateRasterizerState(aFramework->GetDevice()))
		return false;
	if (!CreateSamplerState(aFramework->GetDevice()))
		return false;
...
{
	myContext->OMSetBlendState(myBlendStates[static_cast<int>(aBlendState)], NULL, 0xFFFFFFFF);
}

void CRenderStateManager::SetDepthStencilState(DepthStencilStates aDepthStencilState)
{
	myContext->OMSetDepthStencilState(myDepthStencilStates[static_cast<int>(aDepthStencilState)], 0);
}

void CRenderStateManager::SetRasterizerState(RasterizerStates aRasterizerState)
{
	myContext->RSSetState(myRasterizerStates[static_cast<int>(aRasterizerState)]);
}

void CRenderStateManager::SetSamplerState(SamplerStates aSamplerState)
{
	myContext->PSSetSamplers(0, 1, &mySamplerStates[static_cast<int>(aSamplerState)]);
}
...
bool CRenderStateManager::CreateBlendStates(ID3D11Device* aDevice)
{
	...
	additativeblendDescritption.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
...
bool CRenderStateManager::CreateDepthStencilState(ID3D11Device* aDevice)
{
	...
	myDepthStencilStates[static_cast<int>(DepthStencilStates::DEPTHSTENCILSTATE_DEFAULT)] = nullptr;
	myDepthStencilStates[static_cast<int>(DepthStencilStates::DEPTHSTENCILSTATE_ONLYREAD)] = onlyreadDepthStencilState;
	return true;
}
...
bool CRenderStateManager::CreateRasterizerState(ID3D11Device* aDevice)
{
	...
	myRasterizerStates[static_cast<int>(RasterizerStates::RASTERIZERSTATE_DEFAULT)] = nullptr;
	myRasterizerStates[static_cast<int>(RasterizerStates::RASTERIZERSTATE_WIREFRAME)] = wireframeRasterizerState;
	return true;
}
...
bool CRenderStateManager::CreateSamplerState(ID3D11Device* aDevice)
{
	...
	mySamplerStates[static_cast<int>(SamplerStates::SAMPLERSTATE_TRILINEAR)] = nullptr;
	mySamplerStates[static_cast<int>(SamplerStates::SAMPLERSTATE_POINT)] = pointSamplerState;
	return true;
}
